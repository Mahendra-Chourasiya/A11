...
if openai_api_key and langchain_api_key:
    ...
    def get_session_history(session: str) -> BaseChatMessageHistory:
        if session not in st.session_state:
            ...
            try:
                with open(f"{session}_history.json", "r") as history_file:
                    history_data = json.load(history_file)
                    st.session_state[session] = ChatMessageHistory(messages=history_data)
            except FileNotFoundError:
                st.session_state[session] = ChatMessageHistory()  # Create new if no saved history
        ...
        if user_input:
            try:
                ...
                with open(f"{session_id}_history.json", "w") as history_file:
                    json.dump(session_history.messages, history_file)
